...
/* Description:
 *
 * Helps applications such as QEGui and QE widgets add menu items from the main
 * window menu bar and push buttons to the main window tool bar.
 * Named customisations are saved in .xml files.
 * Any number of customisation .xml files can be read, building up a directory
 * of named customisations.
 * Customisations can be requested by name.
 * Customisations may include other customisations by name.
 * A customisation can be aliased by defining a customisation that only includes
 * another named customisation.
 *
...
// Construct instance of class defining an individual item when none exists
// (for example, a menu placeholder)
//
...
windowCustomisationItem::~windowCustomisationItem() { }  // place holder
...
windowCustomisationItem::windowCustomisationItem(
      ...
      const QEPvInfo& pvInfoIn)                            // PV Name, value and format
   : iAction( NULL )
{
   commonInit();
...
windowCustomisationItem::windowCustomisationItem( const QString dockTitleIn,
                                                  bool /*unused*/ ): iAction( NULL )
{
   commonInit();
...
void windowCustomisationItem::itemAction()
{
   ...
   // Used by both applicationLauncher and QEPvWriteOnce.
   //
   const QString macroSubs = this->getMacroSubstitutions();

   // If the action is associated with a program, launch it.
   // The applicationLauncher takes a VariableManager in order to perfrom
   // any substitutions, so let's give it one.
   //
   VariableManager vm;
   vm.setVariableNameSubstitutions( macroSubs );
   programLauncher.launch( &vm, NULL );

   // If the action is associated with writing a value to a PV, do it.
   //
   if (!pvInfo.pvName.isEmpty()) {
      QEPvWriteOnce* writeAndForget =
         new QEPvWriteOnce (pvInfo.pvName, pvInfo.value, pvInfo.format, this);
...
// (not a placeholder for items the application might add)
//
windowCustomisationMenuItem::windowCustomisationMenuItem(
   ...
   : windowCustomisationItem( launchRequestReceiver, windowsIn, programIn, argumentsIn, pvInfoIn)
{
   type = typeIn;
                                                      ...
                                                      QEPvInfo& pvInfo )
{
   ...
   // Read PV related information
   //
   QDomElement pvElement = itemElement.firstChildElement( "PV" );
   if( !pvElement.isNull() ) {
      // read PV name value and format
      //
      pvInfo.pvName = pvElement.attribute( "Name" );

      QDomElement valueElement = pvElement.firstChildElement( "Value" );
      if( !valueElement.isNull() ) {
         pvInfo.value = valueElement.text();
      }

      QString format = "Default";
      QDomElement formatElement = pvElement.firstChildElement( "Format" );
      if( !formatElement.isNull() ) {
         format = formatElement.text();
      }

      // Convert from text to QEStringFormatting format type.
...
// Add details for a menu item to customisation set
windowCustomisationMenuItem*
windowCustomisationList::createMenuItem( QDomElement itemElement, QStringList menuHierarchy)
{
   ...
   QEPvInfo pvInfo;                        // PV name, value and format
   ...
   if( parseMenuAndButtonItem( itemElement, title, windows, builtIn, program,
                               widgetName, arguments, dockTitle, pvInfo ) )
   {
      if( !dockTitle.isEmpty() ) {
         // Add details for a existing dock menu item to customisation set
         //
         item = new windowCustomisationMenuItem(
                  log, menuHierarchy, title, windowCustomisationMenuItem::MENU_ITEM,
                  requiresSeparator( itemElement ), itemCheckInfo( itemElement ),
                  dockTitle );
      }
         ...
         //
         item = new windowCustomisationMenuItem(
                  log, menuHierarchy, title, windowCustomisationMenuItem::MENU_BUILT_IN,
                  requiresSeparator( itemElement ), itemCheckInfo( itemElement ),
                  builtIn, widgetName );
      }
         ...
         item = new windowCustomisationMenuItem(
                  ...
                  windows, program, arguments, pvInfo );
      }
   }
   return item;
}
...
windowCustomisationButtonItem*  windowCustomisationList::createButtonItem( QDomElement itemElement )
{
   ...
   windowCustomisationButtonItem* item = NULL;

   if( parseMenuAndButtonItem( itemElement, title, windows, builtIn, program,
                               widgetName, arguments, dockTitle, pvInfo ) )
   {
      // If any windows or a program, build an item that holds these
      // (Not sure why these are grouped. Either all options (windows, program, or built-in)
      // should be mutually exclusive, or any mix allowed.
      //
      if( windows.count() || !program.isEmpty() || !pvInfo.pvName.isEmpty())
      {
         // Add details for a button item to customisation set
         //
         item = new windowCustomisationButtonItem(
                  buttonGroup, buttonToolbar, buttonLocation, title, buttonIcon,
                  NULL,  /*!!! needs launch receiver object */
                  windows, program, arguments, pvInfo );
...
void windowCustomisationList::addIncludeCustomisation( QDomElement includeCustomisationElement,
...
QMenu* windowCustomisationList::buildMenuPath( windowCustomisationInfo* customisationInfo,
                                               QMenuBar* menuBar,
                                               const QStringList menuHierarchy )
{
...
void windowCustomisationList::activateDocks()
{
   ...
   QObject::disconnect(toBeActivatedMW, SIGNAL( dockCreated( QDockWidget* ) ),
                       this, SLOT( useDock( QDockWidget* ) ));
...
//==============================================================================
//
windowCustomisationInfo::windowCustomisationInfo () { }
