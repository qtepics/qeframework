...
public:
    ...
    /// This method returns an xml dom element that reflects the detailed
    /// configuration of the widget. It leverages off saveConfiguration,
    /// however it provides a "raw" QDomElement elements; i.e. no file
    /// reading or writing.
    /// Note: this is back to front, i.e. saveConfiguration should leverage
    /// off this method.
    //
    QDomElement getDetailedConfiguration (const QString& tagName);


    /// This method uses the given an xml dom element to configure the widget.
    /// Note: it leverages off restoreConfiguration to acheive this.
    //
    void setDetailedConfiguration (const QDomElement& config);
...
protected:
    ...
    QString persistantName( const QString& prefix ) const;                ///< Returns a string that will not change between runs of the application (given the same configuration)
    ...
    bool useOwnPersistantName;                                            /// The persistant manager need name with embedded meta data.
                                                                          /// Setting this true ensures "simple" names are generated by persistantName.
private:
