...
/**
  ...
  (such as QELabel, QESpinBox, etc), including managing CA data sources.
  It manages CA and PVA data sources through this class.

  Refer to the QEwidget class description for an overview of how QE widgets use QEWidget.

  The VariableManager class manages QEChannel classes that stream updates to the
  CA aware widgets. But the VariableManager class, however, doesn't know how to
  format the data, or how the updates will be used.
  ...
  After construction, a CA/PVA aware widget is activated (starts updating) by VariableManager
   calling itsestablishConnection() function in one of two ways:
   ...
   2) When an QEForm widget is created, resulting in a set of CA/PVA aware widgets being created by loading a UI file
  ...
  No matter which way a CA/PVA aware widget is activated, the establishConnection() function in the CA aware widget is called
  for each variable. The establishConnection() function asks this QEWidget base class, by calling the createConnection()
  function, to perform the tasks common to all CA/PVA aware widgets for establishing a stream of CA data.

  The createVariable() function calls the CA/PVA aware widget back asking it to create an object based on QEChannel.
  This object will supply a stream of CA/PVA update signals to the CA/PVA aware object in a form that it needs. For example a QELabel
  creates a QEString object. The QEString class is based on the QEChannel class and converts all update data to a strings
  which is required for updating a Qt label widget. This class stores the QEChannel based class.

  After the establishConnection() function in the CA/PVA aware widget has called createConnection(), the remaining task of the
...
public:
   ...
   /// Return a reference to one of the QEChannels used to stream CA updates
   ///
   QEChannel* getQcaItem (unsigned int variableIndex) const;
...
protected:
   ...
   QEChannel* createVariable (const unsigned int variableIndex,      ///< Create a CA/PVA connection. do_subscribe indicated if updates should be requested.
                              const bool do_subscribe);              ///< Return a QEChannel if successfull.
   ...
   void deleteQcaItem (const unsigned int variableIndex,             ///< Delete a stream of CA/PVA updates
                       const bool disconnect);

private:
   void clearQcaItems();             // Deallocate and free all QEChannels.

   typedef QList<QEChannel*> ChannelLists;
   ChannelLists channelList;         // CA/PV access - provides a stream of updates.
                                     // One for each variable name used by the QE widgets
};
