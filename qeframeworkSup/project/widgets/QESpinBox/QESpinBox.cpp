...
//------------------------------------------------------------------------------
//
void QESpinBox::usePvNameProperties (const QEPvNameProperties & pvNameProperties)
{
...
// For a spin box a QEChannel that streams real (double) numbers is required.
//
QEChannel *QESpinBox::createQcaItem (unsigned int variableIndex)
{
...
void QESpinBox::establishConnection (unsigned int variableIndex)
{
   ...
   if (qca) {
      ...
      QObject::connect (qca,  SIGNAL (valueUpdated    (const QEFloatingValueUpdate&)),
                        this, SLOT   (setSpinBoxValue (const QEFloatingValueUpdate&)));
...
//
void QESpinBox::connectionUpdated (const QEConnectionUpdate & update)
{
...
/*
    ...
    This is the slot used to recieve data updates from a QEChannel based class.
*/
void QESpinBox::setSpinBoxValue (const QEFloatingValueUpdate & update)
{
   const unsigned int vi = update.variableIndex;
   ...
   if (update.isMetaUpdate && this->autoScaleSpinBox) {
   ...
   // Do nothing more if doing a single shot read
   // (done when not subscribing to get range values).
   //
...
void QESpinBox::setSuffixEgu (QEChannel * qca)
{
   // If using the EGU as the suffix, and the EGU is available, set the suffix
   // to the EGU otherwise clear the suffix.
   //
...
//
void QESpinBox::setDecimalsFromPrecision (QEChannel * qca)
{
   // If using the database precision to determine the number of decimal places,
   // and it is available, then apply it.
   //
...
void QESpinBox::setAddUnitsAsSuffix (bool addUnitsAsSuffixIn)
{
   ...
   QEChannel *qca = getQcaItem (PV_VARIABLE_INDEX);
...
//------------------------------------------------------------------------------
// useDbPrecision
// Note, for most widgets with an 'useDbPrecision' property, the property is
// passed to a QEStringFormatting class where it is used to determine the
// precision when formatting numbers as a string.  In this case, it is used
// to determine the spin box number-of-decimals property.
//
...
{
   return this->useDbPrecisionForDecimal;
}
